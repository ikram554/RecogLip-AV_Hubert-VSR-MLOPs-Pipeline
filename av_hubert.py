...
# os.chdir('C:/content/av_hubert/avhubert/')


def predict(video_path, ckpt_path, user_dir):
    num_frames = int(cv2.VideoCapture(
        video_path).get(cv2.CAP_PROP_FRAME_COUNT))
    ...
    itr = task.get_batch_iterator(dataset=task.dataset(
        gen_subset)).next_epoch_itr(shuffle=False)
    sample = next(itr)
    ...
    # Calculate accuracy here and assign it to the `accuracy` variable
    accuracy = ...

    # Log metrics and artifacts to MLflow
    mlflow.start_run()
    mlflow.log_param("ckpt_path", ckpt_path)
    mlflow.log_param("user_dir", user_dir)
    mlflow.log_artifact(ckpt_path, "model")
    mlflow.log_artifact(user_dir, "model")
    mlflow.log_artifact(video_path, "input")
